...
class AmplitudeQKNeighborsClassifier:
    ...
    def _init_circuit(self, X_train, y_train):
        ...
        # Assuming that training contains all type of classes
        n_classes = len(np.unique(y_train))
        # Needed qubit to represent all the feature of a given element
        n_feature_indexes = int(math.ceil(math.log2(N)))
        # Needed qubit to represent all training inputs
        n_training_indexes = int(math.ceil((math.log2(tr_size))))
        # Needed qubit to represent all the classes
        n_class_indexes = int(math.ceil(math.log2(n_classes)))

        self.qrama = QuantumRegister(
            n_training_indexes + n_feature_indexes + 1, 'qram')  # Ancilla QRAM
        ...
        self.classical_classes = ClassicalRegister(
            n_class_indexes, 'classical_classes')  # n_class_indexes(n_class_indexes)

        self.circuit = QuantumCircuit(self.qrama, self.i, self.knna, self.m, self.r,
                                      self.c, self.classical_r, self.classical_knna, self.classical_classes)
    ...
    def fit(self, X_train, y_train):
        ...
        for idx, x_i, y_i in zip(range(len(X_train)), X_train, y_train):
            ...
            _amplitude_mapper(self.circuit, x_i, self.i,
                              self.i[0:]+self.knna[0:]+self.m[0:]+self.c[0:], self.r[0], self.qrama)
    ...
    def predict(self, X_test, n_shots=8192):
        ...
        _amplitude_mapper(
            self.circuit, X_test[0], self.i, self.i[0:]+self.knna[0:], self.r[0], self.qrama)
        self.circuit.barrier()
        ...
        def post_select(counts): return [(state, occurences) for state, occurences in counts.items(
        ) if state[-1] == '1' and state[-3] == '0']
        postselection = dict(post_select(counts))
        sorted_votes = sorted(postselection.items(),
                              key=lambda x: x[1], reverse=True)

        print(sorted_votes)
        # removing values for knna and r ' 0 1'
        majority_vote = sorted_votes[0][0][0:-4]
        self.circuit = None
