...
def two_binary_complement_gate(value, n, name='value'):
    ...
    # return the binary repr of two_compl.
    bin_two_complement = get_binary_value(int_two_complement, n)
...
def get_hamming_distance_gate(N, name="hamming_distance"):
    # minimum number of qubits to represent the sum of Hamming Distance's 1s
...
class BasisQKNeighborsClassifier:
    ...
    def __init__(self, precision=3, n_neighbors=1):
        ...
        # minimum number of qubits to represent the sum of Hamming Distance's 1s
        self.n = math.ceil(math.log2(self.N+1))+1
    ...
    def _init_circuit(self):
        ...
        self.circuit = QuantumCircuit(
            self.v, self.c, self.x, self.d, self.c1, self.a, self.b)
    ...
    def fit(self, trainings):
        ...
        self.template_circuit.append(basis_encode_dataset(
            trainings, self.N), self.v[0:]+self.c[0:])

        # Appending Hamming Distance Gate --------------------#
        self.template_circuit.append(get_hamming_distance_gate(
            self.N), self.v[0:]+self.x[0:]+self.d[0:]+self.c1[0:])
        # -----------------------------------------------------

        # Appending Subtraction Gate -------------------------#
        #     (di - key) if di < key --> most significant bit is 0, otherwise 1
        self.template_circuit.append(
            subs_gate(self.n), self.d[0:]+self.b[0:]+self.a[0:])
    ...
    def predict(self, test):
        ...
        while minimum <= maximum:
            ...
            # Loading Test
            self.circuit.append(get_binary_value_gate(
                test, self.N, 'test:'), self.x[0:])

            self.circuit.append(two_binary_complement_gate(
                key, self.n, name='key: '+str(key)), self.b[0:])
            ...
            def post_select(counts): return [
                (state, occurences) for state, occurences in counts.items()]
            postselection = dict(post_select(counts))
            postselection = sorted(postselection.items(),
